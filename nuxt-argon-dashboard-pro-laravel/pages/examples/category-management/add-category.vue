...
const submitForm = async () => {
    errorsRef.value = [];
    v$.value.$validate();
    const categoryExist = checkItemExist(formData.name, categoryStore.allCategoriesList);
    ...
    if (v$.value.$error || categoryExist) {
        ...
        try {
            const createCategoryResponse = await categoryStore.createCategory(formData.name, formData.description);

            if (createCategoryResponse.error.value) {
                const errorMessage = createCategoryResponse.error.value.data.errors[0].detail;
                throw new Error(errorMessage);
            } else {
                useToast('success', 'Category created successfuly');
