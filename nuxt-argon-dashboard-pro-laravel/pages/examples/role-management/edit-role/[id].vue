<template>
    <div class="card shadow-lg mx-4 p-3 mt-4">
        <div class="d-flex justify-content-between px-4 pt-3">
            <h5 class="font-weight-bolder mb-0">Edit Role</h5>
        ...
        <div class="mt-4">
            <form role="form" @submit.prevent="$isDemo(roleId) ? $demoMessage('roles') : submitForm()">
...
const { params: { id: roleId } } = useRoute();
...
const getRole = async () => {
    await roleStore.getRole(roleId);
    formData.role = roleStore.currentRole?.name;
}
...
const submitForm = async () => {
    errorsRef.value = [];
    v$.value.$validate();
    const roleExist = checkItemExist(formData.role, roleStore.allRolesList, roleStore.currentRole.name);
    ...
    if (v$.value.$error || roleExist) {
        ...
        try {
            const updateRoleResponse = await roleStore.updateRole(roleId, formData.role);

            if (updateRoleResponse.error.value) {
                const errorMessage = updateRoleResponse.error.value.data.errors[0].detail;
                throw new Error(errorMessage);
            } else {
                useToast('success', 'Role updated successfuly');
