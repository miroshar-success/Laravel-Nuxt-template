<template>
    <ul class="pagination" :class="[size && `pagination-${size}`, align && `justify-content-${align}`]">
      <li class="page-item prev-page" :class="{disabled: value === 1, 'cursor-pointer': value !== 1}">
      ...
      <li v-for="item in range(minPage, maxPage)" :key="item" class="page-item"
        ...
        <a class="page-link" :style="[value === item ? {'color': 'white'} : {'color': '#8392ab'}]" @click="changePage(item)">{{item}}</a>
      </li>
      <li class="page-item next-page" :class="{disabled: value === totalPages, 'cursor-pointer': value !== totalPages}">
